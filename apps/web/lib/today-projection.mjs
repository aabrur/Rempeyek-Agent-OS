...
export function buildTodayProjection(projects) {
  ...
  const unfinishedTasks = [...(project.tasks || [])]
    ...
    .sort((a, b) => priority(a.priority) - priority(b.priority) || (a.status === "pending" ? -1 : 1) || String(a.id).localeCompare(String(b.id)));
  const unresolvedDecisions = (project.decisions || []).filter(decision => !["resolved", "accepted", "rejected"].includes(decision.status));
  const recentArtifacts = [...(project.artifacts || project.files || [])].sort((a, b) => time(b.updatedAt) - time(a.updatedAt)).slice(0, 8);
