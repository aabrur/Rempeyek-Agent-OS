---
name: agentic-os
description: "Use whenever working with Agentic OS as the Windows dashboard for Hermes, OpenClaw, Claude Code, or other local agents. Covers dashboard lifecycle, gateway wrapper behavior on Windows, auth/localhost quirks, and frontend-to-backend control verification. Trigger on agentic-os dashboard issues, Hermes/OpenClaw/Claude Code gateway control from UI, Start/Stop/Restart/Status/Run buttons not working, port-in-use dashboard relaunch, 401 from localhost, gateway command parsing, or daily bridge/telemetry lane visibility."
---
