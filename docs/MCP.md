...
|---|---|---|
| **Supabase mirror** | `apps/web/server.js` (`sbMirrorAgents`) | Optional. `SUPABASE_URL` + `SUPABASE_SERVICE_KEY` in `.env` → agent registry upserted to `aos_agents` (DDL: `supabase/aos_agents.sql`). service_role key is **server-side only**, never in `apps/web/public/`. Filesystem stays the source of truth. |
...
2. **Secrets stay in `.env` / agent keychains** — never in the vault, never in
   `agents.config.json` (it's gitignored but still synced/mirrored), never client-side.
...
Future home for connector adapters (Supabase mirror, GitHub sync, Figma token import)
so `apps/web/server.js` shrinks to routing + serving. Adapters must stay optional and
